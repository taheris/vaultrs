...
/// ## Import Key
/// This endpoint imports existing key material into a new transit-managed
/// encryption key. To import key material into an existing key, see the
/// import_version/ endpoint.
///
/// * Path: {self.mount}/keys/{self.name}/import
/// * Method: POST
/// * Response: N/A
/// * Reference: https://www.vaultproject.io/api-docs/secret/transit#import-key
#[derive(Builder, Debug, Default, Endpoint, Serialize)]
#[endpoint(
    path = "{self.mount}/keys/{self.name}/import",
    method = "POST",
...
pub struct ImportKeyRequest {
    ...
    #[endpoint(skip)]
    pub name: String,
    /// A base64-encoded string that contains two values: an ephemeral 256-bit
    /// AES key wrapped using the wrapping key returned by Vault and the
    /// encryption of the import key material under the provided AES key. The
    /// wrapped AES key should be the first 512 bytes of the ciphertext, and the
    /// encrypted key material should be the remaining bytes.
    ...
    /// A base64-encoded string providing a context for key derivation. Required
    /// if derived is set to true.
...
///
/// Note: Keys whose material was generated by Vault do not support importing
/// key material. Only keys that were previously imported into Vault can import
/// new key material from an external source.
///
/// * Path: {self.mount}/keys/{self.name}/import_version
/// * Method: POST
/// * Response: N/A
/// * Reference: https://www.vaultproject.io/api-docs/secret/transit#import-key-version
#[derive(Builder, Debug, Default, Endpoint, Serialize)]
#[endpoint(
    path = "{self.mount}/keys/{self.name}/import_version",
    method = "POST",
...
pub struct ImportKeyVersionRequest {
    ...
    /// encryption of the import key material under the provided AES key. The
    /// wrapped AES key should be the first 512 bytes of the ciphertext, and the
    /// encrypted key material should be the remaining bytes. See the BYOK
    /// section of the Transit secrets engine documentation for more information
    /// on constructing the ciphertext.
    pub ciphertext: String,
    /// The hash function used for the RSA-OAEP step of creating the ciphertext.
    pub hash_function: Option<HashFunction>,
}
...
/// ## Get Wrapping Key
/// This endpoint is used to retrieve the wrapping key to use for importing
/// keys. The returned key will be a 4096-bit RSA public key.
///
/// * Path: {self.mount}/wrapping_key
/// * Method: GET
/// * Response: GetWrappingKeyResponse
/// * Reference: https://www.vaultproject.io/api-docs/secret/transit#get-wrapping-key
#[derive(Builder, Debug, Default, Endpoint, Serialize)]
#[endpoint(
    path = "{self.mount}/wrapping_key",
    response = "GetWrappingKeyResponse",
